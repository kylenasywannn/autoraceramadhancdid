...
local function applyForceMovement(car, targetPos, cpNumber)
    ...
    if _G.IsTweening then
        -- Landing dengan kecepatan -340 dan jeda 0.25 detik
        part.AssemblyLinearVelocity = Vector3.new(0, -340, 0)   -- <-- DIUBAH: -300 → -340
...
local function cleanPathway()
    ...
    if billboard then table.insert(targets, billboard) end
    ...
    if mountainSmooth then table.insert(targets, mountainSmooth) end
